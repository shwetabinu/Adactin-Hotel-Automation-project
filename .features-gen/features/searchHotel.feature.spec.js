...
test.describe('Search Hotel Functionality', () => {
  ...
  test.describe('To verify search hotel functionality for <searchCriteria>', () => {
    ...
    test('To verify search hotel functionality for same checkin and check out', { tag: ['@searchHotel', '@searchHotelInvalid'] }, async ({ Given, When, Then, authenticatedLoginPage, searchHotelPage }) => { 
      await Given('I am already logged into Adactin Hotel application', null, { authenticatedLoginPage }); 
      await When('I fill the search hotel form with "same checkin and check out" test data', null, { searchHotelPage }); 
      await When('The "search" button is clicked', null, { searchHotelPage }); 
      await Then('Mandatory error message should be displayed for "same checkin and check out"', null, { searchHotelPage }); 
    });
